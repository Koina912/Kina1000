...
API_TOKEN = '7187634138:AAFmD1aGD4MbIFLK6f1COKWSxl-nC4qhtP8'
...
async def add_user(message: Message):
    if message.from_user.id!= ADMIN_ID:
        await message.answer("👲🏿 𝘽𝙨𝙙𝙠 𝘼𝙙𝙢𝙞𝙣 𝙃𝙤 𝙆𝙮𝙖 𝙏𝙪𝙢 ? ☢️🔞")
        return
    ...
    save_authorized_users()
    await message.answer(f"User {user_id} added with authorization period of {authorization_period} days.")
...
async def update_user(message: Message):
    if message.from_user.id!= ADMIN_ID:
        await message.answer("🔞 𝘼𝙙𝙢𝙞𝙣 𝙃𝙤 𝙆𝙮𝙖 𝙏𝙪𝙢 ? 🏵️🦸🏿")
        return
    args = message.text.split()[1:]
    if len(args)!= 2:
    ...
    if user_id in AUTHORIZED_USERS:
        ...
        save_authorized_users()
        await message.answer(f"User {user_id} updated with new authorization period of {new_authorization_period} days.")
    else:
        await message.answer(f"User {user_id} not found.")

async def list_users(message: Message):
    if message.from_user.id!= ADMIN_ID:
        await message.answer("☠️ 𝘽𝙨𝙙𝙠 𝘼𝙙𝙢𝙞𝙣 𝙃𝙤 𝙆𝙮𝙖 𝙏𝙪𝙢 ? 🦸🏿☣️")
        return
    user_list = []
...
async def user_info(message: Message):
    ...
    if user_data:
        ...
        approval_expiry_str = "𝙊𝙥𝙥𝙨 𝙉𝙤𝙩 𝙖𝙥𝙥𝙧𝙤𝙫𝙚𝙙 𝘾𝙤𝙣𝙩𝙖𝙘𝙩 @SATYAM_HK"
